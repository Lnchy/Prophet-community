...
create table if Not exists `seer_admin_info` 	#管理员信息表
(
	`admin_id` int primary key auto_increment,	#管理自增长id
	`admin_user_id` int,									#管理员用户id
	`admin_type_id` int,									#管理员类型id
	`admin_time` datetime DEFAULT NOW()				#管理员创建日期
);

create table if Not exists `seer_admin_type`		#管理员类型表
(
	`admin_type_id` int primary key auto_increment,	#类型自增长id
	`admin_type_name` varchar(20),						#管理员级别名称
	`admin_interest_id` int							  		#管理版块id
);
...
(
	...
	`interset_describe` varchar(300)						#问题类型描述
);
